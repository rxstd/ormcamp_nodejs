...
router.get("/", function (req, res, next) {
  res.render("index", { title: "Evans Admin", user: false });
});

router.get("/login", function (req, res, next) {
  res.render("login", { title: "Evans Admin", error: false });
});
...
router.post("/login", function (req, res, next) {
  ...
  if (userAuth.status == "-1") {
    ...
    res.render("login", { title: "Evans Admin", error: userAuth.error });
  }
  ...
  if (userAuth.status == "1") {
    ...
    res.render("index", { title: "Evans Admin", user: userAuth.data });
  }
});
